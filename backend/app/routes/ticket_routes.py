...
from backend.models.ticket import TicketCreate, TicketUpdate
...
async def create_ticket(ticket: TicketCreate, current_user: User = Depends(get_current_user)):
    ...
    """
    send_email(current_user.email, "Ticket Submitted", email_body)

    return {"msg": "Ticket created", "id": str(new_ticket.id)}

@router.put("/tickets/{ticket_id}")
async def update_ticket(ticket_id: PydanticObjectId, update: TicketUpdate, current_user: User = Depends(get_current_user)):
    ticket = await db.ticket_collection.get(ticket_id)
    if not ticket:
    ...
    """
    send_email(current_user.email, "Ticket Status Updated", email_body)
